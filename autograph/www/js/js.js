...
$(document).ready(function(){
  ...
  function startAnaliz() {
    ...
    if (angle_char==0) {
      ...
      _RESULT_.html_text.push('Общительность и открытость, быстрое восприятие, активность');
    } else if (angle_char==1) {
      _RESULT_.text.push('<strong>Вы интересный собеседник</strong> и всегда объективно излагаете свою позицию.');
      _RESULT_.html_text.push('Объективность, сдержанность и аккуратность в общении.');
    } else {
      _RESULT_.text.push('<strong>В общении Вы осторожны и проявляете самообладание.</strong>');
      _RESULT_.html_text.push('Самообладание и тактичность в общении');
    }
    ...
    if (angle_char==0) {
      ...
      _RESULT_.html_text.push('Утонченная и творческая натура,  предпочтение закрытым мероприятиям');
    } else {
    ...
    //_RESULT_.html_text.unshift('Уникальное сочетание деталей в подписи отражает Вашу индивидуальность.')
    
    _.each(_RESULT_.html_text, function(element, index, list){
      $('.step-5 .text-wrapper .text ul.list').append('<li>'+element+'</li>')
    })

    $('.step-5 .text-wrapper .text').prepend('<p>Уникальное сочетание деталей в подписи отражает Вашу индивидуальность.</p>');
    $('.step-5').prepend('<h2>'+_RESULT_.main+'</h2>');
    
    _.each(_RESULT_.text, function(element, index, list){
      _RESULT_.raw_text += '<p>'+ element +'</p>'
    })
